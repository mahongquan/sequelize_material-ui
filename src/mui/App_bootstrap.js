      ...
      <TableRow key={idx}  className={this.props.classes.row}>
        ...
        <CustomTableCell>
          ...
          <DropdownButton title="" id="id_dropdown3">
            <MenuItem onClick={() => this.onDetailClick(contact.id)}>详细</MenuItem>
            <MenuItem onClick={()=>this.opendlgurl("/rest/updateMethod",this,idx,{id:contact.id})}>更新方法</MenuItem>
            <MenuItem onClick={()=>this.opendlgwait(contact.id)}>全部文件</MenuItem>
            <MenuItem onClick={()=>this.opendlgcheck(contact.id,contact.yiqibh)}>核对备料计划</MenuItem>
            <MenuItem onClick={()=>this.opendlgfolder(contact.id)}>资料文件夹</MenuItem>
            
          </DropdownButton>
        </CustomTableCell>
        <CustomTableCell>{contact.yiqixinghao}</CustomTableCell><CustomTableCell>{contact.channels}</CustomTableCell>
...
<div className={this.props.classes.root}>
    ...
    <Toolbar>
      ...
      <Button  color="inherit" onClick={this.openDlgPacks}>包</Button>
      <Button  color="inherit" onClick={this.openDlgItems}>备件</Button>
      <Button  color="inherit" onClick={this.openDlgCopyPack}>复制包</Button>
    ...
    <DropdownButton title={this.state.user} id="id_dropdown1">
          ...
          onClick={this.openDlgLogin}>登录</MenuItem>
          <MenuItem style={{display:this.state.user==="AnonymousUser"?"none":"block"}} 
          onClick={this.handleLogout}>注销</MenuItem>
       </DropdownButton>
      <InputBase  color="inherit"  onKeyPress={this.keypress} value={this.state.search}  
      ...
      <Button  color="inherit" onClick={this.search}>
       ...
       <Button   color="inherit" style={{margin:"0px 10px 0px 10px"}}  variant="contained" color="primary" onClick={()=>this.handleEdit(null)}>新仪器</Button>
       <Button   color="inherit" variant="contained" onClick={this.openDlgImport}>导入标样</Button>
       <Button   color="inherit"  style={{margin:"0px 10px 0px 10px",display:"none"}}  
        color="primary" onClick={this.openDlgImportHT}>导入合同</Button>
  </Toolbar>
  </AppBar>
 <div align="center" style={{display:this.state.connect_error?"":"none",textAlign: "center",color:"red"}} >!!!!!!!!!!连接错误,服务器未运行!!!!!!!</div>
 <div className={this.props.classes.tableWrapper}>    
<Table  className={this.props.classes.table} ><TableHead><TableRow>
<CustomTableCell><span onClick={this.handleClickFilter}>客户单位</span>
</CustomTableCell>
<CustomTableCell>合同编号</CustomTableCell>
<CustomTableCell><span onClick={this.handleClickFilter}>仪器编号</span></CustomTableCell>
<CustomTableCell>仪器型号</CustomTableCell><CustomTableCell>通道配置</CustomTableCell>
<CustomTableCell>入库时间</CustomTableCell><CustomTableCell>方法</CustomTableCell></TableRow>
</TableHead>
<TableBody>
{contactRows}
</TableBody>
...
<Button id="page_go"  onClick={this.jump}>跳转</Button>
